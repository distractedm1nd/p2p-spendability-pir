...
use spendability_pir_client::witness::WitnessClient;
use spendability_pir_server::ingest::extract_commitments;
use spendability_pir_server::ingest::proto::CompactBlock;
use spendability_pir_server::ingest::LwdClient;
use spendability_pir_server::pir::YpirPirEngine;
use spendability_pir_server::witness::server::{build_router, run_sync_only};
use spendability_pir_server::witness::state::ServerConfig;
...
async fn full_pir_round_trip() {
    ...
    let client = WitnessClient::connect(&base_url, pir_protocol::ZcashNetwork::Main)
        .await
