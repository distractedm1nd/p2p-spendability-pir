...
async fn test_sync_stream_mock() {
    ...
    spendability_pir_server::ingest::sync_blocks(&mut client, 1, 100, |block| {
        let nullifiers =
...
async fn test_incomplete_range_emits_nothing() {
    ...
    let mut client =
        spendability_pir_server::ingest::LwdClient::connect(&[format!("http://{addr}")])
            ...
            .unwrap();
    let mut received = 0;
    let error = spendability_pir_server::ingest::sync_blocks(&mut client, 1, 3, |_| {
        received += 1;
        std::future::ready(Ok::<(), spendability_pir_server::ingest::PipelineError>(()))
    })
    .await
    ...
    assert!(matches!(
        error,
        spendability_pir_server::ingest::PipelineError::UnexpectedHeight {
            expected: 2,
            actual: 3
        }
    ));
...
async fn test_follow_new_blocks_mock() {
    ...
    let follow_handle = tokio::spawn(async move {
        spendability_pir_server::ingest::follow_blocks(
            &mut client,
            5,
            hash_for(5),
            0,
            ...
            |event| {
                ...
                async move {
                    tx.send(event).await.map_err(|_| {
                        spendability_pir_server::ingest::PipelineError::ConsumerDropped
                    })
                }
            },
        )
    ...
    match event {
        spendability_pir_server::ingest::BlockEvent::NewBlock(block) => {
...
async fn test_follow_reorg_mock() {
    ...
    match &event {
        ...
        spendability_pir_server::ingest::BlockEvent::Reorg { rollback_to } => {
            assert_eq!(*rollback_to, 2)
        }
    }
