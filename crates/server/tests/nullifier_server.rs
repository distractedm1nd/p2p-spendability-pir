use nullifier_pir::{hash_to_bucket, PirEngine, BUCKET_BYTES, NUM_BUCKETS};
...
use spendability_pir_server::nullifier::server::{build_router, run_sync_only};
use spendability_pir_server::nullifier::state::ServerConfig;
use spendability_pir_server::pir_stub::StubPirEngine;
...
fn make_config(lwd_addr: SocketAddr, data_dir: &std::path::Path) -> ServerConfig {
    ServerConfig {
        zcash_network: nullifier_pir::ZcashNetwork::Main,
        target_size: nullifier_pir::TARGET_SIZE,
...
async fn test_server_startup_sync_mode() {
    ...
    let app_state = Arc::new(spendability_pir_server::nullifier::state::AppState::new(
        config, engine,
    ));
    ...
    let resp = client
        ...
        .unwrap();
    assert_eq!(resp.status(), 200);
    let scenario: nullifier_pir::YpirScenario = resp.json().await.unwrap();
...
async fn test_server_sync_to_serving() {
    ...
    assert!(matches!(phase, nullifier_pir::ServerPhase::Serving));
...
async fn test_server_query_after_sync() {
    ...
    let found = bucket_data
        .chunks_exact(nullifier_pir::ENTRY_BYTES)
...
async fn test_server_metadata_after_sync() {
    ...
    let meta: nullifier_pir::SpendabilityMetadata = resp.json().await.unwrap();
    assert_eq!(meta.latest_height, 30);
    assert_eq!(meta.earliest_height, 1);
    assert_eq!(meta.num_nullifiers, 90);
    assert!(matches!(meta.phase, nullifier_pir::ServerPhase::Serving));
}
...
async fn test_server_snapshot_save_load() {
    ...
    let restored = spendability_pir_server::nullifier::snapshot_io::load_snapshot(tmp.path())
        .await
        .unwrap();
...
async fn test_server_follow_new_block() {
    ...
    let metadata = nullifier_pir::SpendabilityMetadata {
        zcash_network: nullifier_pir::ZcashNetwork::Main,
        nullifier_pool: nullifier_pir::IRONWOOD_POOL.into(),
        dataset_version: nullifier_pir::DATASET_VERSION,
        ...
        phase: nullifier_pir::ServerPhase::Serving,
    };
    app_state.live_pir.store(Arc::new(Some(
        spendability_pir_server::nullifier::state::PirState {
            engine_state,
            metadata,
        },
    )));
