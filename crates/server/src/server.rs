use crate::ingest::{BlockEvent, LwdClient, PipelineError, ValidatedBlock};
use axum::http::StatusCode;
use axum::routing::get;
use axum::Router;
use nullifier_pir::HashTableDb;
use pir_protocol::{PirEngine, ServerPhase, ZcashNetwork, CONFIRMATION_DEPTH};
...
pub enum ServerError {
    ...
    Nullifier(#[from] crate::nullifier::server::ServerError),
    #[error("witness server error: {0}")]
    Witness(#[from] crate::witness::server::ServerError),
    #[error("chain client error: {0}")]
    Client(#[from] crate::ingest::ClientError),
    #[error("chain pipeline error: {0}")]
    Pipeline(#[from] crate::ingest::PipelineError),
    #[error("chain task failed: {0}")]
    Join(#[from] tokio::task::JoinError),
    #[error(transparent)]
    Ironwood(#[from] crate::ingest::IronwoodError),
    #[error(transparent)]
    IronwoodEndpoint(#[from] crate::ingest::EndpointError),
    #[error(transparent)]
...
pub fn create_app_states<NfP: PirEngine, WitP: PirEngine>(
    ...
    Arc<crate::nullifier::state::AppState<NfP>>,
    Arc<crate::witness::state::AppState<WitP>>,
) {
    let nullifier = Arc::new(crate::nullifier::state::AppState::new(
    ...
    let witness = Arc::new(crate::witness::state::AppState::new(
        crate::witness::state::ServerConfig {
            ...
            lwd_urls: config.lwd_urls.clone(),
            listen_addr: config.listen_addr,
            window_shard_limit: crate::witness::state::DEFAULT_WINDOW_SHARD_LIMIT,
        },
        wit_engine,
    ));
    (nullifier, witness)
...
pub async fn run_with_states_until<NfP: PirEngine + 'static, WitP: PirEngine + 'static>(
    config: CombinedConfig,
    nf_state: Arc<crate::nullifier::state::AppState<NfP>>,
    wit_state: Arc<crate::witness::state::AppState<WitP>>,
    ...
    let router = Router::new()
        ...
        .nest(
            "/nullifier",
            crate::nullifier::server::build_router(nf_state.clone()),
        )
        .nest(
            "/witness",
            crate::witness::server::build_router(wit_state.clone()),
        );
    ...
    crate::ingest::require_ironwood_tree_state(&mut client, config.zcash_network, follow_height)
        .await?;
...
async fn sync_both<NfP: PirEngine, WitP: PirEngine>(
    ...
    let mut hashtable = match crate::nullifier::snapshot_io::load_snapshot(&nf_dir).await {
        Ok(db) => db,
        Err(crate::nullifier::snapshot_io::SnapshotIoError::Io(error))
            if error.kind() == std::io::ErrorKind::NotFound =>
        {
            HashTableDb::new()
        }
        Err(error) => return Err(crate::nullifier::server::ServerError::from(error).into()),
    };
    let nf_from = hashtable
        .latest_height()
        .map(|height| height + 1)
        .unwrap_or_else(|| crate::ingest::nu6_3_activation_height(config.zcash_network));

    let (mut tree, wit_from) = match crate::witness::snapshot_io::load_snapshot(&wit_dir).await {
        Ok(tree) => {
            let from = tree
                .latest_height()
                .map(|height| height + 1)
        ...
        Err(crate::witness::snapshot_io::SnapshotIoError::Io(error))
            if error.kind() == std::io::ErrorKind::NotFound =>
        {
            crate::witness::server::prepare_tree(
                &mut client,
                target_height,
        ...
        Err(error) => return Err(crate::witness::server::ServerError::from(error).into()),
    };
    ...
    if sync_from <= target_height {
        ...
        crate::ingest::sync_blocks(&mut client, sync_from, target_height, |block| {
            let result: Result<()> = (|| {
                apply_block(
                    &block,
...
fn apply_block(
    ...
    hashtable: Option<&mut HashTableDb>,
    tree: Option<&mut CommitmentTreeDb>,
    context: &'static str,
) -> Result<(usize, usize)> {
    let nullifiers = hashtable
        .as_ref()
        .map(|_| crate::ingest::extract_ironwood_nullifiers(&block.block))
        .transpose()?;
    let parsed_commitments = tree
        ...
        .map(|_| {
            ...
            Ok::<_, crate::ingest::ParseError>((commitments, prior_tree_size))
        })
        .transpose()
        .map_err(crate::witness::server::ServerError::from)?;

    if let (Some(tree), Some((_, prior_tree_size))) = (tree.as_deref(), &parsed_commitments) {
        crate::witness::server::validate_prior_tree_size(
            tree,
            block.height(),
    ...
    if let (Some(db), Some(nullifiers)) = (hashtable, &nullifiers) {
        db.insert_block(block.height(), block.hash, nullifiers)?;
    }
    if let (Some(tree), Some((commitments, _))) = (tree, &parsed_commitments) {
...
fn rebuild_both<NfP: PirEngine, WitP: PirEngine>(
    ...
    let nf_pir = crate::nullifier::server::rebuild_pir(
        &*nf_state.engine,
        hashtable,
        &nf_state.scenario,
        config.zcash_network,
    ...
    let wit_pir = crate::witness::server::rebuild_pir(
        &*wit_state.engine,
        tree,
        &wit_state.scenario,
        anchor_height,
...
async fn save_both(
    ...
    crate::nullifier::snapshot_io::save_snapshot(hashtable, &config.data_dir.join("nullifier"))
        .await
        .map_err(crate::nullifier::server::ServerError::from)?;
    crate::witness::snapshot_io::save_snapshot(tree, &config.data_dir.join("witness"))
        .await
        .map_err(crate::witness::server::ServerError::from)?;
    Ok(())
}
...
mod tests {
    use super::*;
    use crate::ingest::proto::{ChainMetadata, CompactBlock, CompactOrchardAction, CompactTx};
