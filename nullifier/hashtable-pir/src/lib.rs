...
use spend_types::{hash_to_bucket, Nullifier, BUCKET_CAPACITY, NUM_BUCKETS};
...
mod tests {
    ...
    fn pir_bytes_are_raw_nullifiers() {
        ...
        assert_eq!(bytes.len(), spend_types::DB_BYTES);
        let bucket = hash_to_bucket(&nf) as usize;
        assert_eq!(&bytes[bucket * spend_types::BUCKET_BYTES..][..32], &nf);
    }
